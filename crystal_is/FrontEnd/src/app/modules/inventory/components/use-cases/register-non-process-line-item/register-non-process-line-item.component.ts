...
@Component({
  selector: 'app-register-non-process-line-item',
  templateUrl: './register-non-process-line-item.component.html',
  styleUrls: ['./register-non-process-line-item.component.css']
})
export class RegisterNonProcessLineItemComponent implements OnInit {
  ...
  private raw_material_service:NonProcessLineItemService) { }
    ...
    registerRawMaterial(){
      ...
      this.purchase_history_service.savePurchaseHistory(purchase_history_data).subscribe(response=>{
        ...
        this.raw_material_service.saveNonProcessLineItem(raw_material_data).subscribe(response=>{
